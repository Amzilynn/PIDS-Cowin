...
from core.portrait_engine import LivePortraitEngine
from processing.lip_sync_manager import AudioLipSync, IdleAnimator

# =========================================================================
# CONFIGURATION
# =========================================================================
# Pathing relative to the ROOT run script
ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SOURCE_IMAGE = os.path.join(ROOT_DIR, "assets", "ava.jpg")
DASHBOARD_FILE = os.path.join(ROOT_DIR, "demo", "index.html")
FPS = 6 # Hardware-locked floor for perfect timing
LIP_SYNC_ADVANCE = 0.15 # Pull lips forward by 150ms to match neural/network lag
PORT = 8027
    ...
    while True:
        try:
            ...
            if is_speaking and audio_ready and speech_timeline:
                # Sync lips to ACTUAL audio playback start time
                # We add LIP_SYNC_ADVANCE to account for neural smoothing delay
                elapsed_speech = time.time() - speech_start_time + LIP_SYNC_ADVANCE
...
async def speak_pipeline(sid, text, voice):
    ...
    try:
        ...
        ffmpeg_bin = os.path.join(ROOT_DIR, "ffmpeg-master-latest-win64-gpl-shared", "bin", "ffmpeg.exe")
        if not os.path.exists(ffmpeg_bin):
            # Try the root one as fallback
            ffmpeg_bin = os.path.join(ROOT_DIR, "ffmpeg.exe")
