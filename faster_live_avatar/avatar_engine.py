...
class LivePortraitEngine:
    def __init__(self, source_image_path: str):
        ...
        cfg = InferenceConfig()
        cfg.checkpoint_F = "base_repo/pretrained_weights/liveportrait/base_models/appearance_feature_extractor.pth"
        cfg.checkpoint_M = "base_repo/pretrained_weights/liveportrait/base_models/motion_extractor.pth"
        cfg.checkpoint_W = "base_repo/pretrained_weights/liveportrait/base_models/warping_module.pth"
        cfg.checkpoint_G = "base_repo/pretrained_weights/liveportrait/base_models/spade_generator.pth"
        cfg.checkpoint_S = "base_repo/pretrained_weights/liveportrait/retargeting_models/stitching_retargeting_module.pth"
    ...
    def render_frame(self, lip_open: float, lip_spread: float, lip_pucker: float, eye_blink: float, head_tilt: float = 0.0):
        with torch.no_grad():
            ...
            target_exp[0, 19, 1] += float(lip_open) * 0.12  
            target_exp[0, 17, 1] += float(lip_spread) * 0.08 
            target_exp[0, 20, 1] += float(lip_pucker) * 0.15 
