...
from session import EvaluationThread, load_delegues, save_conversation, update_delegue_score, _score_to_level, conversation_loop
...
class TrainingSessionManager:
    ...
    def start_session(self, delegue_id: int, delegate_type: str):
        if self.is_active:
        ...
        self.delegue["type"] = delegate_type
        ...
        self.conv_thread = threading.Thread(
            ...
            args=(self.delegue, self.eval_thread, self.client, self.retriever, self.user_input_queue),
            daemon=True
        )
    ...
    def _run_conversation(self, delegue, eval_thread, client, retriever, user_input_queue):
