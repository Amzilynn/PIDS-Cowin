...
def start_training(req: StartRequest):
    try:
        res = manager.start_session(
            delegue_id=req.delegue_id,
            delegate_type=req.delegate_type
        )
        return res
...
def chat_message(req: ChatRequest):
    ...
    manager.user_input_queue.put({"type": "text", "content": req.message})
    return {"status": "received"}

@router.post("/voice_trigger")
def voice_trigger():
    if not manager.is_active:
        raise HTTPException(status_code=400, detail="Aucune session active.")
    manager.user_input_queue.put({"type": "voice_trigger"})
