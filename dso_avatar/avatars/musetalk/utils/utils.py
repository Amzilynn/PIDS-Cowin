...
def load_all_model(
    ...
    if unet_model_path is None:
        unet_model_path = os.path.join(base_dir, "models", "musetalkV15", "unet.pth")
    if unet_config is None:
        unet_config = os.path.join(base_dir, "models", "musetalkV15", "musetalk.json")
    
    vae_path = os.path.join(base_dir, "models", vae_type)
    
    # Fallback to alternative VAE name if needed
    if not os.path.exists(vae_path) and vae_type == "sd-vae":
        vae_path = os.path.join(base_dir, "models", "sd-vae-ft-mse")
